...
TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384
    │     │        │              │
    │     │        │              └─ HKDF/PRF 해시 + 키 파생에 쓰는 해시
    │     │        └──────────────── 대칭 암호 + AEAD 모드
    │     └───────────────────────── 서버 인증(서명) 알고리즘
    └─────────────────────────────── 키 교환 알고리즘
```

여기서 실무적으로 중요한 것은 **각 부분이 실패했을 때 무엇을 잃는가** 입니다.
...
TLS 1.3에서는 문자열이 짧아졌습니다.

```
TLS_AES_256_GCM_SHA384    ← 대칭 암호와 해시만 명시
```

키 교환은 **항상 (EC)DHE**, 서명 알고리즘은 별도 확장으로 협상하기 때문입니다. 즉 TLS 1.3은 **전방 비밀성이 없는 조합을 프로토콜 차원에서 제거**했습니다.

### 1.3 전방 비밀성 (Forward Secrecy)

RSA 키 교환(TLS 1.2 이하)의 구조적 문제:

```
[RSA 키 교환]
클라이언트 → RSA_공개키(pre-master secret) → 서버
              ↑ 서버 개인키로만 복호화 가능

문제: 공격자가 트래픽을 수년간 저장해 두었다가
      나중에 서버 개인키를 입수하면 → 저장된 모든 세션이 복호화된다.
```
...
무엇을 잃는가: **암호화는 여전히 동작합니다.** 그래서 겉으로는 아무 문제가 없어 보입니다. 잃는 것은 **인증(authentication)** 이고, 그 결과 중간자가 자기 인증서를 제시해도 그대로 통과합니다. 공격자는 트래픽을 복호화·변조한 뒤 그대로 중계할 수 있습니다. 즉 **"HTTPS를 쓰는데 HTTP보다 위험한"** 상태가 됩니다 — 개발자가 안전하다고 믿기 때문입니다.

`HostnameVerifier { _, _ -> true }` 도 동일한 부류입니다. 인증서 체인은 검증하지만 **도메인 일치를 확인하지 않으므로**, 공격자가 자신이 정당하게 발급받은 `evil.com` 인증서로 `api.mybank.com` 을 위장할 수 있습니다.
...
```
Client --TLS--> ALB --HTTP(평문)--> ECS Task
```
...
flowchart TB
    ...
    MASTER --> CATS["client_application_traffic_secret"]
    MASTER --> SATS["server_application_traffic_secret"]
    MASTER --> EXP["exporter_master_secret"]
...
- **핸드셰이크 트래픽 키가 애플리케이션 키보다 먼저 만들어집니다.** 그래서 TLS 1.3은 `Certificate` 부터 **암호화해서 전송**합니다(TLS 1.2는 인증서가 평문). 수동 도청자가 어떤 서버에 접속했는지 알기 어려워집니다 — 다만 SNI는 여전히 평문이며, ECH(Encrypted Client Hello)가 이를 해결하려는 확장입니다.
...
Intermediate CA
    ↑ 서명
Root CA (트러스트 스토어에 존재)
```

JVM은 `PKIXValidator` 로 다음을 순서대로 확인합니다.
...
1. **서비스 메시**(Istio/Linkerd): 사이드카가 mTLS를 투명하게 처리하고 인증서를 자동 회전. 애플리케이션 코드 변경 없음.
2. **SPIFFE/SPIRE**: 워크로드 신원(SVID)을 자동 발급, 수명 수 시간.
3. **애플리케이션 직접 구현**: 소규모에서만 현실적. 아래 4장 예제.

### 3.4 LB에서의 TLS 종료 판단

```
[패턴 A] TLS 종료 (Termination)
Client --TLS--> LB --HTTP--> App     구현 간단, 내부 평문

[패턴 B] TLS 재암호화 (Re-encryption)
Client --TLS--> LB --TLS--> App      LB가 L7 처리 + 내부 암호화. 실무 권장

[패턴 C] TLS 통과 (Passthrough)
Client -----------TLS-----------> App  E2E 암호화, LB는 L4만. WAF/라우팅 불가
```

| 판단 기준 | 권장 |
|----------|------|
| 공개 웹 서비스, 개인정보 없음 | A (종료) |
| 개인정보/결제 데이터 취급 | **B (재암호화)** |
| 규제상 E2E 요구, 클라이언트 인증서 검증을 앱에서 | C (통과) |
| WAF·경로 라우팅·헤더 조작 필요 | A 또는 B (C는 불가) |
...
server:
  ...
  ssl:
    ...
    key-alias: app
    # TLS 1.3만 허용. 레거시 클라이언트가 있으면 TLSv1.2 추가
...
class InternalController {
    ...
    fun getData(request: HttpServletRequest): ResponseEntity<Data> {
        ...
        if (clientCn !in ALLOWED_SERVICES) {
    ...
    companion object {
        private val ALLOWED_SERVICES = setOf("order-service", "payment-service")
    }
}
```
...
class MutualTlsClientConfig {
    ...
    @Bean
    fun sslBundleContext(
        @Value("\${app.mtls.keystore-path}") keyStorePath: String,
        @Value("\${app.mtls.keystore-password}") keyStorePassword: String,
        @Value("\${app.mtls.truststore-path}") trustStorePath: String,
        @Value("\${app.mtls.truststore-password}") trustStorePassword: String,
    ): SSLContext {
        val keyStore = loadStore(keyStorePath, keyStorePassword)
        val trustStore = loadStore(trustStorePath, trustStorePassword)
    ...
    fun internalWebClient(
        ...
        val sslContext = SslContextBuilder.forClient()
            .keyManager(kmf)                      // 클라이언트 인증서 제시
            .protocols("TLSv1.3")
            .build()                              // trustManager 미지정 = 시스템 기본 검증 유지
    ...
    private fun loadStore(path: String, password: String): KeyStore =
...
> Spring Boot 3.1+ 에서는 **SSL Bundles** 로 이 보일러플레이트를 줄일 수 있습니다. `spring.ssl.bundle.pem.internal.*` 을 정의하고 `RestClient.Builder#apply(sslBundle)` 로 주입하면, 인증서 파일 변경 시 **핫 리로드**까지 지원됩니다. 신규 프로젝트라면 SSL Bundles를 우선 검토하세요.

### 4.4 자체 서명 인증서를 "올바르게" 다루기

`TrustAll` 대신 해당 CA만 신뢰합니다.

```kotlin
/** 사설 CA만 추가로 신뢰하는 TrustManager. 공인 CA 검증은 그대로 유지된다. */
fun trustManagerWithInternalCa(caCertPath: String): TrustManagerFactory {
    val ca = FileSystemResource(caCertPath).inputStream.use {
        CertificateFactory.getInstance("X.509").generateCertificate(it) as X509Certificate
    }
    // 시스템 기본 cacerts를 복사한 뒤 사설 CA를 추가한다
...
**금지 코드 목록** — 코드 리뷰·정적 분석 룰로 차단할 것:

```kotlin
// 아래 패턴이 발견되면 머지 차단
X509TrustManager { checkServerTrusted 가 빈 구현 }
HostnameVerifier { _, _ -> true }
NoopHostnameVerifier.INSTANCE
TrustSelfSignedStrategy()  // 프로덕션 코드에서
"-Dcom.sun.net.ssl.checkRevocation=false"
```

CI에 다음 정도의 검사만 넣어도 대부분 걸립니다.
...
class CertificateExpiryMonitor(
    ...
    /** 만료까지 남은 일수를 메트릭으로 노출 → Prometheus 알림 룰로 연결 */
    @Scheduled(cron = "0 0 9 * * *")
    fun checkAll() {
        endpoints.forEach { host ->
            runCatching { daysUntilExpiry(host) }
                .onSuccess { days ->
                    meterRegistry.gauge("tls.cert.days_until_expiry",
                        listOf(Tag.of("host", host)), days)
...
```
certbot renew (cron, 12시간마다)
  → 만료 30일 이내면 갱신
  → 성공 시 --deploy-hook 실행
       ├─ PEM → PKCS12 변환
       └─ 애플리케이션 리로드 (또는 SSL Bundle 핫 리로드)
```
...
openssl pkcs12 -export \
  ...
  -out /opt/app/keystore.p12 -name app \
...
- [../main/08-asymmetric-crypto-and-signature.md](../main/08-asymmetric-crypto-and-signature.md) — ECDHE와 인증서 서명의 수학적 배경
- [비대칭키 암호와 전자서명](../main/08-asymmetric-crypto-and-signature.md) — Diffie-Hellman 키 교환
