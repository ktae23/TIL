...
SHA-256("passworD")  = 6b3a55e0261b0304143f805a24924d0c1c44524821305f31d9277843b8a10f4e
                       └─ 마지막 글자 하나 바꿨을 뿐인데 완전히 다른 값 (눈사태 효과)
```
...
```
SHA-256 처리 속도 (대략치)
  서버 CPU 1코어  : 초당 약 1,000만 회
  RTX 4090 1장    : 초당 약 100억 회 (10 GH/s)
  ASIC 채굴기      : 초당 약 100조 회 (100 TH/s)
```

8자리 영문+숫자 비밀번호의 전체 경우의 수는 `62^8 ≈ 2.18 × 10^14`입니다. RTX 4090 한 장이면 **약 6시간**, 여러 장이면 몇십 분입니다. 실제 비밀번호는 사전 단어 기반이 대부분이라 훨씬 빨리 뚫립니다. 해시의 속도는 무결성 검증에서는 **장점**이지만 비밀번호 저장에서는 **치명적 단점**이고, 여기서 비밀번호 해싱이 일반 해싱과 갈라집니다.
...
salt 없음:
  ...
  user_b: SHA256("password123") = ef92b778...   ← 같은 해시! 레인보우 테이블 1회 조회로 둘 다 뚫림

salt 있음:
  user_a: SHA256("7Kx9mQ" + "password123") = a3f5...
  user_b: SHA256("pL2vNw" + "password123") = c81b...   ← 사용자마다 별도 브루트포스 필요
```

**salt는 비밀이 아닙니다.** 검증에 반드시 필요하므로 DB에 해시와 나란히 평문으로 저장합니다. 목적은 "숨기는 것"이 아니라 **"사전 계산과 병렬 공격을 무력화하는 것"** 입니다. BCrypt·Argon2는 salt를 해시 문자열 안에 포함해 저장합니다. 요건은 **사용자마다 고유**, **암호학적 난수**(`SecureRandom`, `java.util.Random` 금지), **16바이트 이상**, **비밀번호 변경 시 재생성**입니다.
...
pepper의 가치는 **DB 덤프 유출 시나리오**에 있습니다. `hash = Argon2id(password + pepper, salt)`로 저장하면 pepper는 DB에 없고 애플리케이션 서버에만 존재하므로, SQL Injection이나 백업 파일 유출로 DB만 털린 공격자는 salt와 hash를 얻어도 **오프라인 브루트포스 자체를 시도할 수 없습니다.**
...
```mermaid
flowchart LR
    B["PBKDF2 (2000)<br/>반복 횟수만 조절<br/>→ GPU 병렬화에 약함"]
    B --> C["BCrypt (1999)<br/>Blowfish 기반, 4KB 메모리<br/>→ 초기 GPU에 저항"]
    C --> D["scrypt (2009)<br/>메모리 하드<br/>→ ASIC/GPU 비용 급증"]
    D --> E["Argon2 (2015)<br/>PHC 우승, 메모리·시간·병렬성<br/>독립 조절 → 현재 권장"]
```

| 알고리즘 | 방어 대상 | 파라미터 | 권장값 (OWASP 기준) |
|----------|----------|---------|-------------------|
| **PBKDF2** | CPU 시간 | 반복 횟수 | HMAC-SHA256 60만 회↑. FIPS 필요 시에만 |
| **BCrypt** | CPU + 4KB 메모리 | cost (log₂) | cost 12 (≈250ms). **72바이트 입력 제한** |
| **scrypt** | CPU + **메모리** | N, r, p | N=2^17, r=8, p=1 (약 128MB) |
| **Argon2id** | CPU + **메모리** + 병렬화 | m, t, p | **m=64MiB, t=3, p=4** — 1순위 |

**왜 메모리가 중요한가?** GPU는 코어가 수천 개지만 코어당 메모리는 적고, ASIC도 메모리를 늘리면 단가가 급등합니다. 알고리즘이 64MB를 요구하면 코어 수와 무관하게 동시 인스턴스 수가 메모리로 제한됩니다. **메모리 하드니스(memory-hardness)** 는 공격자의 하드웨어 우위를 직접 깎는 방어입니다.

**Argon2 변종** — `Argon2d`(데이터 의존 접근, GPU 저항 최고지만 사이드채널 취약), `Argon2i`(데이터 독립, 사이드채널 안전), **`Argon2id`**(하이브리드, OWASP 권장).

파라미터는 **반드시 실측하세요.** 위 값은 출발점일 뿐이고, 실제 운영 서버에서 벤치마크해 200~500ms에 맞추는 것이 맞습니다.
...
`upgradeEncoding()`이 이 패턴의 핵심입니다. `DelegatingPasswordEncoder`는 저장된 해시의 `{id}`가 현재 `idForEncode`와 다르면 `true`를 반환합니다. 로그인할 때마다 MAU 비율만큼 자동 전환되고(보통 3개월이면 80% 내외), 잔여 계정은 비밀번호 재설정 안내로 마무리하면 됩니다.
...
// ❌ 4. 예측 가능한 salt (순차값 / 시각 / SecureRandom 아닌 난수)
String salt = String.valueOf(user.getId());
String salt = new Random().toString();

// ❌ 5. 자체 제작 스트레칭 — 충돌 위험 + 병렬화 방어 없음
String h = password;
...
// ❌ 8. work factor를 너무 낮게 (기본 생성자는 10, 4는 거의 무방비)
new BCryptPasswordEncoder(4);

// ❌ 9. 비밀번호 최대 길이 제한 — 패스프레이즈를 막는다. 최소 8, 최대 64~128 권장
@Size(max = 20) private String password;

// ❌ 10. 강한 알고리즘 옆에 약한 레거시 fallback 존치 (Ashley Madison 사례)
```
