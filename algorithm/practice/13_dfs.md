...
그래프를 탐색하는 방법 중 하나로, 가능한 **깊이 들어갔다가** 더 이상 갈 곳이 없으면 **돌아오는** 방식입니다.
...
private static boolean dfsCycle(int node, int parent, List<List<Integer>> graph, boolean[] visited) {
    ...
    for (int next : graph.get(node)) {
        if (!visited[next]) {
            if (dfsCycle(next, node, graph, visited)) return true;
        } else if (next != parent) {
            return true;  // 방문했는데 부모가 아니면 사이클
        }
    }
    return false;
}
...
private static boolean dfsPath(int node, int end, List<List<Integer>> graph, boolean[] visited) {
    ...
    for (int next : graph.get(node)) {
        if (!visited[next]) {
            if (dfsPath(next, end, graph, visited)) return true;
        }
    }
    return false;
}
...
1. **재귀**가 직관적이지만 스택오버플로우 주의
2. **visited 배열**로 무한 루프 방지
3. 백트래킹: visited를 다시 **false로 복원**
4. 시간 복잡도: O(V + E)
