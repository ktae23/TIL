...
3. 마크다운 미리보기 (선택):
   ...
   **AskUserQuestion 도구**를 사용하여 미리보기 여부를 확인합니다:
   - header: "미리보기"
   - question: "작성된 마크다운 파일을 브라우저에서 미리보기 하시겠습니까?"
   - options:
     - "예": HTML로 변환하여 브라우저에서 미리보기
     - "아니오": 미리보기 건너뛰고 Git 작업 진행

   사용자가 "예"를 선택하면:
   ```bash
   # 임시 HTML 파일 생성 및 브라우저에서 열기
   # pandoc이 설치되어 있으면 pandoc 사용, 없으면 grip 또는 기본 변환 사용

   # pandoc 사용 시
   pandoc -f markdown -t html -s --metadata title="TIL Preview" \
     -o /tmp/til-preview.html "{생성된_파일_경로}"
   open /tmp/til-preview.html

   # 또는 Python grip 사용 시 (GitHub 스타일 렌더링)
   grip "{생성된_파일_경로}" --export /tmp/til-preview.html
   open /tmp/til-preview.html

   # 또는 간단한 방법: VS Code에서 미리보기
   code "{생성된_파일_경로}"
   ```
...
→ 마크다운 파일 생성
→ [미리보기] 예 / 아니오  → "예" 선택 시 브라우저에서 HTML 미리보기
→ git commit → git push
```
