...
<body>
    ...
    # 4. viewer.html 저장 (기존 TIL Viewer)
    output_html = os.path.join(OUTPUT_DIR, "viewer.html")
    with open(output_html, "w", encoding="utf-8") as f:
        f.write(html_content)

    # 5. README.md를 index.html로 변환
    readme_path = os.path.join(TIL_PATH, "README.md")
    if os.path.exists(readme_path):
        with open(readme_path, "r", encoding="utf-8") as f:
            readme_content = f.read()
        readme_json = json.dumps(readme_content, ensure_ascii=False)
        readme_json = readme_json.replace("</", "<\\/").replace("<!--", "<\\!--")
...
<body>
    ...
    <script src="assets/lib/highlight/highlight.min.js"></script>
    <script>
        const README_CONTENT = {readme_json};
        marked.setOptions({{
            highlight: (code, lang) => {{
        ...
        document.getElementById('content').innerHTML = marked.parse(README_CONTENT);
